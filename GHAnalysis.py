...
class Data:
    ...
    def __init(self, dict_address: str):
        ...
        json_list = []
        for root, dic, files in os.walk(dict_address):## 获取文件目录下所有json文件
            for f in files:
                if f[-5:] == '.json':
                    json_path = f
        ...
        for i in json_list:
            ...
            self.__4Events4PerP[i['actor']['login']][i['type']] = self.__4Events4PerP[i['actor']['login']].get(i['type'], 0)+1
            if not self.__4Events4PerR.get(i['repo']['name'], 0):
                self.__4Events4PerR.update({i['repo']['name']: {}})
            self.__4Events4PerR[i['repo']['name']][i['type']] = self.__4Events4PerR[i['repo']['name']].get(i['type'], 0)+1
